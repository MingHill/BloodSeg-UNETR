...
from monai.networks.blocks import (
    ...
    UnetrPrUpBlock,
    UnetrUpBlock,
)  # https://docs.monai.io/en/stable/networks.html#unetr-block
...
class CustomUNETR(nn.Module):
    def __init__(
        ...
        self.feat_size = [
            self.encoder_config.image_size // self.patch_size,
            self.encoder_config.image_size // self.patch_size,
        ]
        self.config = {
    ...
    def forward(self, input):  # input shape: [16, 16, 64, 64] (B, C, H, W)
        ...
        # enc1 shape: torch.Size([16, 4, 64, 64]) (B, FS, H, W)
        enc1 = self.encoder1(input)

        # proj(x2) shape: torch.Size([16, 192, 16, 16])
        # enc2 shape: torch.Size([16, 8, 128, 128])
        x2 = hidden_states_no_cls[3]
        enc2 = self.encoder2(self.proj_feat(x2))

        # torch.Size([16, 192, 16, 16])
        dec2 = self.proj_feat(last_hidden_state_no_cls)
...
if __name__ == "__main__":
    # Original VITMAE config (retrieve from logs)
    vitmaeconfig = {
    ...
    pretrained_model_path = (
        "/home/mhill/Projects/cathepsin/logs/vitmae-grid/02/model.pth"
    )
    ...
    # Loading Train Data
    train_data = np.load(
        "/home/mhill/Projects/cathepsin/data/unet_training_dataset.npz"
    )
    ...
    train_dataloader = DataLoader(
        dataset=train_dataset, batch_size=16, collate_fn=unet_valid_collate
    )

    # Load Valid Data
    valid_data = np.load(
        "/home/mhill/Projects/cathepsin/data/unet_validation_dataset.npz"
    )
    ...
    valid_dataloader = DataLoader(
        dataset=valid_dataset, batch_size=16, collate_fn=unet_valid_collate
    )

    # torch.Size([16, 16, 64, 64]) (B, C, H, W)
    train_next = next(iter(train_dataloader))

    # label_next shape : torch.Size([16, 64, 64])
    valid_next = next(iter(valid_dataloader))
    ...
    trainer = UNETR_TRAINER(
        model=unet_model, optimizer=optimizer, criterion=criterion, device="cuda"
    )
