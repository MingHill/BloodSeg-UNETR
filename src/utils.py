...
def extract_mae(model, dataset, image_index, device, denormalize=True):
    ...
    with torch.inference_mode():
        ...
        )  # assuming model handles normalization internally
    ...
    mask = mask.unsqueeze(-1).repeat(
        1, 1, model.config.patch_size**2 * model.config.num_channels
    )
    ...
    mae = {
        ...
        "image_reconstructed_visible": image_reconstructed_visible.squeeze(
            dim=0
        ).numpy(),
    }
