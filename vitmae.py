...
if __name__ == '__main__':
    ...
    train_dataset = CustomDataset(training_dataset, config["NUM_CHANNELS"])
    valid_dataset = CustomDataset(validation_dataset, config["NUM_CHANNELS"])
    test_dataset = CustomDataset(testing_dataset, config["NUM_CHANNELS"])
    ...
    logging.info(f"Train -> {len(train_dataset) * 16}, {len(train_dataloader) * 16} (samples, batches)")
    logging.info(f"Valid -> {len(valid_dataset) * 16}, {len(valid_dataloader) * 16} (samples, batches)")
    logging.info(f"Test  -> {len(test_dataset) * 16},  {len(test_dataloader) * 16}  (samples, batches)")
