...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(config["LOG_DIR"] + "info.md"),
    ...
    logging.info(
        f"Train -> {len(train_dataset)}, {len(train_dataloader)} (samples, batches)"
    )
    logging.info(
        f"Valid -> {len(valid_dataset)}, {len(valid_dataloader)} (samples, batches)"
    )
    logging.info(
        f"Test  -> {len(test_dataset)},  {len(test_dataloader)}  (samples, batches)"
    )
    ...
    logging.info(
        f"Parameters: {sum(param.numel() for param in unet_model.parameters())}"
    )
    logging.info(
        f"Trainable parameters: {sum(p.numel() for p in unet_model.parameters() if p.requires_grad)}\n"
    )
